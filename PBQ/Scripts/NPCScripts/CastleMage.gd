...
func _process(delta):
	if player_is_near == true:
		if(Input.is_action_just_pressed("interaction_button")):
			...
			PlayerData.Player_Information.spoken_to_castle_mage = true
		if talking_to_player == false:
			level_text.text = "T - Talk to Castle Mage"
		elif talking_to_player == true:
			level_text.text = "The Castle has been attacked, you must stop the threat. /T-cont."
