...
func _process(delta):
	if player_is_near == true:
		if(Input.is_action_just_pressed("interaction_button")):
			...
			PlayerData.Player_Information.spoken_to_lighthouse_mage = true
		if talking_to_player == false:
			level_text.text = "T - Talk to the LightHouse Mage"
		elif talking_to_player == true:
			level_text.text = "We're okay, but check the Castle. /T-cont."
