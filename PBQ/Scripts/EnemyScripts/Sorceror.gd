...
func _physics_process(delta):
	...
	#----Attack Section----#
	#need to make it so attacks from range and close
	#if ranged, still move around to avoid player attacks
	...
	if is_attacking_player == true:
		if attacking == true:
			if attack == 1:
				get_node("AnimationTreePlayer").blend3_node_set_amount("AttackBlend", -1)
				get_node("AnimationTreePlayer").oneshot_node_start("SOneShot")
			elif attack == 2:
				get_node("AnimationTreePlayer").blend3_node_set_amount("AttackBlend", 0)
				get_node("AnimationTreePlayer").oneshot_node_start("SOneShot")
			elif attack == 3:
				get_node("AnimationTreePlayer").blend3_node_set_amount("AttackBlend", 1)
				get_node("AnimationTreePlayer").oneshot_node_start("SOneShot")

func sorceror_attack1():
	var area = $Area
	...
	for body in bodies:
		...
		if body.has_method("_hit"):
			...
			print ("zombie damage was: ", damage)
	can_attack_timer = 0.0
	attack_timer = 0.0

func sorceror_attack2():
	var area = $Area
	var bodies = area.get_overlapping_bodies()
	var damage
	damage = zombie_damage
...
func zombie_die():
	...
	PlayerData.Player_Information.beaten_sorceror_boss = true
	queue_free()
